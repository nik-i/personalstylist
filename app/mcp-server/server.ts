...
function buildMcpServer(): McpServer {
  ...
  server.tool(
    "search_garments",
    "Search the user's wardrobe by attributes. All filters optional. Returns id, thumbnailPath, and all classification fields.",
    {
    ...
    async (args) => {
      ...
      const items = await readDb.wardrobeItem.findMany({ where });
      return {
        content: [{ type: "text" as const, text: JSON.stringify(items.map(formatGarment)) }],
      };
    }
  );
